...
import { disciplinaService } from "../disciplina/disciplinaService.js";
...
class TurmaService {
  ...
  async createTurma(data) {
    ...
    if (!codigo || !disciplinaId || !professorId || !periodo) {
      throw new HttpError(
        400,
        "Todos os campos (codigo, disciplinaId, professorId, periodo) são obrigatórios.",
      );
    }
    ...
    if (Number.isNaN(idDisciplinaInt) || Number.isNaN(idProfessorInt)) {
      throw new HttpError(
        400,
        "Os IDs de disciplina e professor devem ser números válidos.",
      );
    }
    ...
    const isProfessor = professor.papeis.some((up) => up.nome === "professor");

    if (!isProfessor) {
      throw new HttpError(
        403,
        `Usuário ${professorId} não possui o papel '${PAPEL_PROFESSOR}' e não pode ministrar a turma.`,
      );
    }
  ...
  async updateTurma(id, data) {
    ...
    if (data.professorId) {
      ...
      if (Number.isNaN(idProfessorInt))
        throw new HttpError(400, "professorId inválido.");
      ...
      try {
        ...
        if (!isProfessor) {
          throw new HttpError(
            403,
            `Usuário ${idProfessorInt} não possui o papel 'Professor'.`,
          );
        }
      } catch (e) {
        if (e.status === 404)
          throw new HttpError(404, "Novo Professor não encontrado.");
        throw e;
      }
